## 业务定制
- 支持mysql，抽取SQL脚本，定制MySQLDataVerticle(使用vertx提供的mysql异步驱动，暂不持支其他数据库)
  
  `` 使用mysql，先初始化db_schema.sql脚本，然后在conf.json里面配置相关信息，通过type属性指定所使用的数据库``
- 添加API请求日志记录，方便日后数据分析
- 优化访问控制和参数传递，使之更加简洁
- 添加缓存机制（无缓存、缓存、自动缓存--根据请求频率自动缓存）
- 优化限流
- 优化负载

# 代码优化
fork VX-API-Gateway 项目后，vertx进行了升级，对代码进行了小的优化和规范化，修复了一些小bug
