...
public class VxApiServerURLPollingPolicy {
     ...
     * 创建轮询策略
     *
     * @param urls 服务连接的URL集,不能为空,size大于1,正确的URL地址
     * @throws NullPointerException  服务连接集合空是抛出
     * @throws MalformedURLException 不是正确的URL是抛出
     */
    ...
    public VxApiServerURLInfo getUrl(String ip) {
        ...
        if (haveService) {
            ...
            if (index == null || !availableMap.get(index)) {
                urlInfo = getUrl();
    ...
    private void updateCurIndex() {
        if (!haveService) {
            ...
            if (availableMap.get(curIndex + 1) != null && availableMap.get(curIndex + 1)) {
                curIndex++;
                return;
            }
			// 标记当前坐标到结束是否有可用的地址
