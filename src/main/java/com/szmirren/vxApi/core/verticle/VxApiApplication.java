...
public class VxApiApplication extends AbstractVerticle {
     ...
     * 全局IP黑名单
     */
    private LinkedHashSet blackIpSet = new LinkedHashSet<>();
    /**
     * HTTP的路由服务
     */
    ...
    public void start(Promise<Void> startFuture) throws Exception {
        try {
            ...
            if (appOption == null) {
                ...
                Future<Void> eventFuture = Future.future(future -> {
                    // 注册操作地址
                ...
                CompositeFuture.all(httpFuture, httpsFuture, eventFuture).onSuccess(res -> {
                    LOG.info("实例化应用程序-->成功");
                    startFuture.complete();
                }).onFailure(err -> {
                    LOG.error("实例化应用程序-->失败:", err.getCause());
    ...
    public void createHttpsServer(Handler<AsyncResult<Void>> createHttps) {
        ...
        createFuture.future().onSuccess(check -> {
            if (check) {
                // 404页面
            ...
            LOG.error("执行创建https服务器-->失败:无效的证书或者错误的路径:如果证书存放在conf/cert中,路径可以从cert/开始,示例:cert/XXX.XXX", err.getCause());
            createHttps.handle(Future.failedFuture(err.getCause()));
        });
    }
    ...
    public void addRouteToRouter(VxApi api, Router router, Map<String, List<Route>> routeMaps, Handler<AsyncResult<Boolean>> result) {
        vertx.executeBlocking(fut -> {
            ...
            initExceptionHandler(api, exRoute);
            routes.add(exRoute);
     ...
     * 初始化与后端服务交互
     *
     * @param isNext 下一步还是结束(也就是说如有后置处理器inNext=true,反则false)
     * @param api
     * @param route
     * @throws Exception
     */
