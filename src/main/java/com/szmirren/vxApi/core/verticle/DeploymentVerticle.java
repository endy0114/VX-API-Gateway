...
public class DeploymentVerticle extends AbstractVerticle {
    ...
    public void unDeploymentAPP(Message<JsonObject> msg) {
        ...
        Future<Void> unDeployFuture = Promise.<Void>promise().future();
        unDeployFuture.onComplete(handle -> {
    ...
    public void startAllAPI(Message<JsonObject> msg) {
        ...
        if (body != null && body.size() > 0) {
            applicationApiMaps.computeIfAbsent(appName, k -> new HashSet<>());
            // 启动所有API
     ...
     * @param serverType      服务的类型1=http,2=htpps,3=webSocket
     * @param handler
     */
